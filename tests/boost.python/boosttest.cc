...
public:
  ...
  { cout << "In Fiction::Fiction(const Fiction&)" << endl; count_ = rhs.count_; }
...
public:
  ...
  virtual string say() { return "HMMMMMMMmmmm!"; }
};
...
std::string call_say(ABC& b) { return b.say(); }
int call_bam(ABC& abc, int a, int b) { return abc.bam(a, b); }
...
public:
  ABCWrap(PyObject* self_)
    : self(self_) {}
  std::string say() { return call_method<std::string>(self, "say"); }
...
public:
  ...
  void abc_says_else()
  {
    ...
    while (iter != iter_e)
    {
      cout << "Item " << i << " says else: '" << (*iter)->say_else(fact_) << "'" << endl;
      ++iter;
      ++i;
    }
  }
...
{
  class_<Fiction>("Fiction", no_init)
    .def("get_count", &Fiction::get_count)
    ;
  ...
  class_<ABC, ABCWrap, boost::noncopyable>("ABC")
    .def("say", &ABCWrap::say)
    .def("say_what", &ABCWrap::say_what)
    .def("say_else", &ABCWrap::say_else) //, &ABCWrap::default_say_else)
    .def("get_hmm", &ABCWrap::get_hmm, return_internal_reference<>())
    ;

  class_<derived, bases<ABC> >("derived")
    .def("say", &derived::say)
    .def("bam", &derived::bam)
