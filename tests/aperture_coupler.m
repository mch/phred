...
B = 7.899e-9; % Ku band waveguide narrow-wall size

global C, EPS, MU;
C = 3e8; % Speed of light, approximatly

EPS = 8.854185336732027e-12;
...
r = 0.2; % Input reflection coefficient
c = 0.7079; % Voltage coupling factor, 3dB
k1 = f1*2*pi * sqrt(MU*EPS);
k2 = f2*2*pi * sqrt(MU*EPS);
kc = pi/A;
...
xx = (4*pi*Pmx)/(a*b*l_g) * sin(pi*x*a)^2;

bz = (-1) * (pi*l_g*Pmz)/(a^3*b) * cos(pi*x*a)^2;

by = (4*pi*l_g*Pey)/(a*b*l_0^2) * sin(pi*x*a)^2;
