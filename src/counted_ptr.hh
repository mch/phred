...
/* Stolen from http://ootips.org/yonat/4dev/counted_ptr.h, 
 * http://ootips.org/yonat/4dev/smart-pointers.html
 *
 * Modified to support an initial non-zero reference count.
 */
...
public:
  ...
  explicit counted_ptr(X* p = 0, unsigned int init_ref_count = 1) // allocate a new counter
    : itsCounter(0) {if (p) itsCounter = new counter(p, init_ref_count);}
  ~counted_ptr()
  {release();}
